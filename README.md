...
### 키워드 정리
- catch : 내가 오류 잡아서 복구할거임
- throws : 나 안 잡을거야 던질거임
- finally : 오류가 발생하던 말던, 오류를 밖으로 던지던간에 마지막에 이건 꼭 실행하고 끝내 

### 예외 계층화
- 장점
  - 부모 예외를 잡거나 던지면, 자식 예외도 똑같이 할 수 있음 
- 방법
  - 디테일 한것을 제일 먼저(가장 중요한 오류 -> 중요한 오류의 부모 오류 -> exception 오류로 공통된 오류들 막기) 
  - 마지막에 제일 공 통된 부모 exception 두기

### 실무 처리 방안
- 생각보다 실무에서 처리할 수 있는 예외가 별로 없다(상대 서버의 문제, 네트워크 선의 문제 등) 내가 복구 불가능 -> throws로 밖으로 던져야 한다 
- 체크 예외를 사용할 경우, 계속 밖으로 던져야 한다 -> 지저분한 코드(일일이 예외를 다 명시해야 함)
-> 언체크 예외를 사용하자
  - 언체크 예외는 throws를 선언하지 않아도 됨
  - 따라서, try로 잡지 않으면 자동으로 밖으로 던져줌
  - 해결할 수 없는 예외는 모아서 다른 곳에서 공통으로 처리 


### try-with-resources
- try가 끝나면 자동으로 close를 호출해줌 -> finally가 없어도 된다는 말
